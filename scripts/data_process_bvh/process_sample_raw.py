...
dict_keys = ["gender", "mocap_framerate", "poses", "trans"]
...
joints_to_use = np.arange(0, 72).reshape((-1, 3))[joints_to_use].reshape(-1)

all_sequences = ["bvh_sample"]   # mixamo_bvh/bvh_sample/punch_motions/sample05.npz
...
def read_single_sequence(folder, seq_name):
    ...
    for subject in tqdm(subjects):
        ...
        for action in actions:
            ...
            vid_name = f"{seq_name}_{subject}_{action[:-4]}"   #  bvh_sample_punch_motions_sample05
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dir", type=str, help="dataset directory", default="/data/soumya/mixamo_bvh/"
    )
    parser.add_argument(
        "--out_dir", type=str, help="dataset directory", default="/data/soumya/mixamo_out/"
    )
    parser.add_argument(
    ...
    db_file = osp.join(out_path, "bvh_db_smplh.pt")

    db = read_data(args.dir, sequences=args.sequences)
     
    
    print(f"Saving BVH dataset to {db_file}")
